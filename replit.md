...
- Narrative cache: `fightCacheTable.narrative` (jsonb) stores the full FightResult on first generation, canonicalized to teamA-first perspective. On subsequent identical matchups (regardless of team1/team2 order), both `POST /api/fights` and `POST /api/fights/stream` return the cached payload in <100ms instead of regenerating via AI. Symmetric flipping inverts `winner` and swaps `team1Hp`/`team2Hp` per round when caller's team1 is canonical teamB. Upset Mode bypasses the cache for users wanting a fresh narrative. Pre-existing cache rows without a narrative get backfilled on next AI run. Result: rematches drop from ~30s to ~0.07s.
