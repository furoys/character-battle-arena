...
- **Daily lineup**: 10 curated matchups posted every day at **8:00 PM ET** (rolls over via `America/New_York`, DST-aware — see `artifacts/api-server/src/lib/dailyPool.ts` `getDailyDateString`). Pool is **202 entries**. Selection is **day-of-week themed** via `artifacts/api-server/src/lib/dailyThemes.ts`: Sun=DC SUNDAY, Mon=MARVEL MONDAY, Tue=TEAM-UP TUESDAY, Wed=WILDCARD WEDNESDAY, Thu=THROWDOWN THURSDAY (horror/villains), Fri=ANIME FRIDAY, Sat=SMASH SATURDAY (gaming). Each theme has its own pre-shuffled bucket (deterministic per-theme mulberry32 seed) built by keyword inference over each entry's id/title. Daily picks walk `DAILY_LINEUP_SIZE` entries through the bucket at `offset = (weekIndex * 10) % bucketSize` — **every matchup in a theme bucket appears exactly once before any repeat on that day-of-week** (e.g. Marvel bucket = 60 → 6 Marvel Mondays before any Marvel matchup repeats; this kills the user-reported "Magneto vs Prof X repeating" pattern that the old pure Fisher–Yates allowed via clustering). A small per-day mulberry32 reshuffle varies display order day to day within the chosen 10. `getThemedDailyMatchupsForDate` falls back to filling from the wildcard bucket if a themed bucket is ever <10. `GET /api/daily` now returns `theme: { key, label, blurb }` alongside `matchups`; the frontend header (`artifacts/fight-club/src/pages/daily.tsx`) replaces the static "Daily Matchups" with `theme.label` + italic blurb. Lineup-stability invariant is unchanged — `ensureDailyRows` still locks the first materialized rows for a date in by serial PK; the themed selector only runs on dates that have no rows yet.
